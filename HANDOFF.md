...
- ⚠ **공개 캡처 배포 = PowerShell 스크립트 `remold_capture.ps1`(exe 폐기, 2026-09-01)**: 2026-09-01 오전 Defender ML 정의 업데이트(v1.457.439.0) 이후 서명 없는 메모리스캐너 **exe가 `Trojan:Win32/Sabsik.FL.A!ml`로 다운로드·디스크 차단**됨. 해시 무관(최초 통과본 713bd33도 재차단), 롤백·재컴파일 무의미(런타임 메모리읽기 행위가 트리거). → **exe 및 다운로드 버튼 webdist에서 제거**, 대신 `files/remold_capture.ps1`(자립형·데이터 내장·자기승격, `scratchpad/gen_capture_ps.py`로 생성) 배포. **텍스트 스크립트라 PE 트로이 판정 대상 아님** → 다운로드 통과. `deploy_web.js`가 ps1을 webdist에 복사. 실행법은 계산기 cap-tool 안내(`powershell -ExecutionPolicy Bypass -File ...`). 잔여 리스크: 빡센 PC는 실행 시 AMSI/ASR로 걸릴 수 있음(님 PC·Ctrl+R은 정상). exe를 살리려면 유료 코드사이닝 or MS 오탐신고(`microsoft.com/wdsi/filesubmission`, 해시단위·1~3일).
  ...
  - **파일명 타임스탬프(2026-09-01)**: 출력 파일 `gf2_remold_<UTC yyyyMMddThhmmssZ>.csv`(로거 형식). 덮어쓰기 아님 → 이력 보존·로거와 시각 짝맞춤 용이. 계산기 안내=「가장 최근 파일」. 데스크톱 Ctrl+R은 TEMP에 auto-import + 바탕화면에 타임스탬프 사본도 저장.
  - **개수불확실 경고 제거 + 재실행 권장으로 대체(2026-09-01)**: per-item 「개수 불확실」 경고 삭제(신뢰성 낮음). 대신 **frac 0.8~0.88(올림된 위험구간, 진짜 (k+1)개무리 0.89+ 미만) 항목 수 ≥2이면** "참조노이즈 의심 N건 — 창고 새로 열고 재실행 권장" 안내(clean scan에선 0건). 노이즈 항상 양수라 재실행(창고 새로 열기)하면 감소 → 정확도↑. 데스크톱은 done.json `noisy`→토스트.
  ...
  - **개수판정 경계 1.8배(2026-09-01, 최종)**: `owned = round(occ/base)` → **`floor(occ/base + 0.2)`**(경계 frac 0.8). 개당 출현 ~17-18(base=median). **참조노이즈**(장착·캐시 아이템은 +최대 0.72×base 추가 출현)로 1개짜리가 occ 1.5~1.72배까지 뜸 → round(경계1.5배)는 이걸 2개로 과다. 반면 2개짜리는 occ 1.89~2.0배(개당 17~18 편차)라 floor(경계2.0배)는 이걸 1개로 과소. **경계를 공백(1.78~1.88배)에 둬야** 정확 → `floor(occ/base+0.2)`. **인게임 검증**: occ30=1개, occ34/35/36=2개 전부 일치. 잠복경고 밴드 `frac 0.73~0.87`(노이즈최대 0.72와 2개무리 0.89 사이 = 진짜 애매구간, 현재 0건). 히스토리: 0.5→floor→최종 0.8경계.
  - **✅ 로거 대조 검증(2026-09-01, 2계정)**: 배포판 메모리 캡처를 gfl2logger 리몰딩 CSV(서버 실물, 고유 UID)와 순서무시·개수포함 대조. **서브계정(base18): 311/311(100%)**. **본계정(base14, 465종/557γ): 깨끗한 스캔 465/465(100%)**, 단 일반 캡처는 461/465(99.1%)로 4건 +1과다(전부 공격강화3). 원인=**참조노이즈(장착·자주참조 스탯이 순간적으로 occ 더 뜸)는 항상 양수(측정 min0 max2, 음수0), 가끔 특정항목 +12까지 튐** → 그 순간 그 항목만 +1. 규칙 자체는 정확(clean scan 100%). **팁**: 창고 새로 연 직후 캡처 or 여러 번 떠서 최소값 채택 = 노이즈0 → 100%. 대조 패턴: 로거 kind=M&lv=3 γ필터, `(s1, sorted(s2,s3))` 정규화 Counter 비교(순서무시 필수 — 로거는 보조옵 원본순서, 캡처는 정렬).
